...
function sessionsReport() {
  ...
  const recent = files.slice(0, 5).map((f) => {
    let project = "unknown";
    try {
      const firstLine = readFileSync(f.filePath, "utf8").split("\n", 1)[0];
      const cwd = JSON.parse(firstLine).cwd;
      if (cwd) project = basename(cwd);
    } catch {
      // cwd를 못 읽으면 unknown으로 표시
    }
...
async function askSession(sessionId, cwd, question, questionMessageId) {
  ...
  try {
    ...
    const stdout = await runClaude(
      ["-p", "--resume", sessionId, question, "--output-format", "json"],
      cwd,
    );
    ...
    // resume은 세션을 포크하므로 새 session_id로 매핑을 갱신해 연속 질문을 지원
    writeState((state) => {
      state.sessions ??= {};
      state.messages ??= {};
      ...
      if (sent) state.messages[String(sent.message_id)] = { sessionId: newSessionId, cwd };
    });
  } catch (error) {
