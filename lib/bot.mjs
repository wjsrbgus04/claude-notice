...
if (!existsSync(CONFIG_PATH)) {
  console.error(`Config file not found (설정 파일이 없습니다): ${CONFIG_PATH}`);
  process.exit(1);
}

const config = JSON.parse(readFileSync(CONFIG_PATH, "utf8"));
if (!config.botToken || !config.chatId) {
  console.error("config.json requires botToken/chatId (config.json에 botToken/chatId가 필요합니다).");
  process.exit(1);
}
...
// ---------- i18n ----------
// config.lang이 "en"이면 영어, 그 외(미설정 포함)는 한국어 (하위 호환)
...
const MSG = {
  ko: {
    ...
    statusHeader: "최근 세션 현황:",
    noHistory: "세션 기록이 없습니다.",
    sessionsHeader: "최근 세션 기록:",
    askBusy: "⚠️ 이 세션에 대한 질문이 이미 실행 중입니다. 끝나면 다시 시도해 주세요.",
    fileNotFound: (sid) => `❌ 세션(${sid}…)의 기록 파일을 찾을 수 없습니다.`,
    askForwarded: (sid) => `⏳ 세션(${sid}…)에 질문을 전달했습니다. 응답을 기다리는 중…`,
    emptyAnswer: "(빈 응답)",
    askFailed: (err) => `❌ 질문 처리에 실패했습니다:\n${err}`,
    help: [
    ...
    notLinked: "이 메시지는 세션과 연결되어 있지 않습니다. /ask <질문> 을 사용해 주세요.",
    askUsage: "사용법: /ask <질문>",
    noSessionYet: "아직 기록된 세션이 없습니다. Claude Code 작업이 한 번 끝난 뒤 사용해 주세요.",
    cmdStatus: "최근 세션 현황 (진행 중/완료/확인 필요)",
    cmdSessions: "최근 세션 기록 5개",
    cmdAsk: "가장 최근 세션에 이어서 질문",
    cmdHelp: "도움말",
    shortDesc: "Claude Code 작업 알림·현황 조회·원격 질문 봇",
    longDesc:
  ...
  en: {
    ...
    statusHeader: "Recent session status:",
    noHistory: "No session history found.",
    sessionsHeader: "Recent session history:",
    askBusy: "⚠️ A question for this session is already running. Please try again when it finishes.",
    fileNotFound: (sid) => `❌ Could not find the transcript file for session ${sid}….`,
    askForwarded: (sid) => `⏳ Question sent to session ${sid}…. Waiting for the response…`,
    emptyAnswer: "(empty response)",
    askFailed: (err) => `❌ Failed to process the question:\n${err}`,
    help: [
      "This is the Claude Code Notice bot.",
      "",
      "/status — recent session status (running / done / needs attention)",
      "/sessions — five most recent sessions",
      "/ask <question> — ask a follow-up to the most recent session",
      "",
      "Or **reply** to a task notification to continue that exact session.",
    ].join("\n"),
    notLinked: "This message isn't linked to a session. Use /ask <question> instead.",
    askUsage: "Usage: /ask <question>",
    noSessionYet: "No sessions recorded yet. Try again after a Claude Code task finishes.",
    cmdStatus: "Recent session status (running / done / needs attention)",
    cmdSessions: "Five most recent sessions",
    cmdAsk: "Ask a follow-up to the most recent session",
    cmdHelp: "Help",
    shortDesc: "Claude Code task alerts, session status, and remote follow-up questions",
    longDesc:
      "Sends a notification when a Claude Code task finishes; reply to a notification to ask a follow-up in that session. Send /status to check session status.",
  },
}[config.lang === "en" ? "en" : "ko"];
...
function relativeTime(epochSeconds) {
  ...
  if (diff < 60) return MSG.justNow;
  if (diff < 3600) return MSG.minutesAgo(Math.floor(diff / 60));
  if (diff < 86400) return MSG.hoursAgo(Math.floor(diff / 3600));
  return MSG.daysAgo(Math.floor(diff / 86400));
}

const STATUS_LABEL = {
  running: MSG.statusRunning,
  done: MSG.statusDone,
  attention: MSG.statusAttention,
};
...
function sessionsReport() {
  ...
  if (recent.length === 0) return MSG.noHistory;
  return `${MSG.sessionsHeader}\n\n${recent.join("\n")}`;
}
...
async function askSession(sessionId, cwd, question, questionMessageId) {
  if (runningSessions.has(sessionId)) {
    await sendText(MSG.askBusy, questionMessageId);
    return;
  }
  ...
  try {
    ...
    if (!sessionFile) {
      await sendText(MSG.fileNotFound(sessionId.slice(0, 8)), questionMessageId);
      return;
    }
    const resolvedCwd = transcriptCwd(sessionFile) || cwd;

    await sendText(MSG.askForwarded(sessionId.slice(0, 8)), questionMessageId);
    ...
    const answer = result.result?.trim() || MSG.emptyAnswer;
    ...
    await sendText(MSG.askFailed(String(error.message).slice(0, 1000)), questionMessageId);
  } finally {
...
async function handleMessage(msg) {
  if (msg.chat?.id !== ALLOWED_CHAT_ID) {
    log(`ignoring disallowed chat_id: ${msg.chat?.id}`);
    return;
  }
  ...
  if (replyId != null) {
    ...
    await sendText(MSG.notLinked, msg.message_id);
    return;
  }

  if (text === "/start" || text === "/help") {
    await sendText(MSG.help);
    return;
  }
  if (text === "/status") {
  ...
  if (text.startsWith("/ask")) {
    ...
    if (!question) {
      await sendText(MSG.askUsage);
      return;
    }
    const state = readState();
    const sid = state.lastSessionId;
    if (!sid) {
      await sendText(MSG.noSessionYet);
      return;
    }
...
async function main() {
  ...
  log(`bot started: @${me.username} (allowed chat_id: ${ALLOWED_CHAT_ID})`);

  // "/" 입력 시 자동완성 메뉴와 봇 프로필 설명 등록 (실패해도 봇 동작에는 지장 없음)
  try {
    await api("setMyCommands", {
      commands: [
        { command: "status", description: MSG.cmdStatus },
        { command: "sessions", description: MSG.cmdSessions },
        { command: "ask", description: MSG.cmdAsk },
        { command: "help", description: MSG.cmdHelp },
      ],
    });
    await api("setMyShortDescription", { short_description: MSG.shortDesc });
    await api("setMyDescription", { description: MSG.longDesc });
  } catch (error) {
    log("failed to register commands/description:", error.message);
  }
    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (update.message) {
          ...
          handleMessage(update.message).catch((e) => log("message handling error:", e.message));
        }
      }
    } catch (error) {
